...
export default function OrganizationForm({ initialData, onSuccess, onClose, isOpen }: any) {
  ...
  const form = useForm<IFormValues>({
    defaultValues: { title: "", address: "", email1: "", email2: "", website: "", description: "", contactNo: "", telephone_number: "", facebook_url: "", twitter_url: "", instagram_url: "", linkdin_url: "", location_url: "", logo: null },
  });
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialData) {
        ...
        setLogoPreview(null);
        form.reset({ title: "", address: "", email1: "", email2: "", website: "", description: "", contactNo: "", telephone_number: "", facebook_url: "", twitter_url: "", instagram_url: "", linkdin_url: "", location_url: "", logo: null });
      }
    }
  }, [initialData, isOpen]);
...
const Field = ({ name, label, icon, placeholder, type = "text", rules }: any) => (
  ...
  return (
    <>
      <div onClick={handleClose} className={`fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-sm transition-opacity duration-300 ${isOpen ? "opacity-100 visible" : "opacity-0 invisible"}`} />
      <div className={`fixed inset-0 z-[101] flex items-center justify-center p-4 transition-all duration-300 ${isOpen ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}>
        <div className="w-full max-w-3xl bg-white rounded shadow-md border border-gray-200 overflow-hidden font-mukta max-h-[90vh] flex flex-col">
          <ConfigProvider theme={{ token: { colorPrimary: primaryColor, borderRadius: 4 } }}>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="px-6 py-4 space-y-4 overflow-y-auto">
                {/* Logo */}
                <div className="flex flex-col items-center pb-3 border-b border-dashed border-gray-200">
                  <div onClick={() => fileInputRef.current?.click()} className="w-24 h-24 rounded-full border-2 border-dashed flex items-center justify-center overflow-hidden bg-gray-50 cursor-pointer hover:bg-gray-100 transition-all" style={{ borderColor: logoPreview ? primaryColor : "#e5e7eb" }}>
                    {logoPreview ? <img src={logoPreview} alt="Logo" className="w-full h-full object-cover" /> : <Camera size={30} className="text-gray-300" />}
                  </div>
                ...
                <div className="w-full space-y-1">
                  ...
                  <Controller control={form.control} name="description" render={({ field }) => (
                    <textarea {...field} rows={3} placeholder="Organization description..." className="w-full rounded border border-gray-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-1 resize-none" style={{ borderColor: undefined }} />
                  )} />
                </div>

                {/* Social Links */}
                <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider border-t pt-3">Social Media Links</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
