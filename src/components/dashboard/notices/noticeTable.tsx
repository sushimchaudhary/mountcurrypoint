...
export default function NoticeTable({ onEdit, refreshTrigger, searchQuery = "" }: any) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await Promise.all(ids.map((id) => NoticeServices.deleteDetails(id)));
  ...
  const downloadPDF = () => {
    const doc = new jsPDF();
    doc.text("Notices", 14, 15);
    autoTable(doc, {
      head: [["S.N.", "Title", "Status"]],
  ...
  return (
    <div className="space-y-3">
              ...
              {loading ? (
                  ...
                  <td colSpan={6} className="text-center py-16">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-[#364a63]">
                        {searchQuery ? "No results." : "No notices yet."}
                      </span>
                    </div>
                  </td>
                </tr>
                ...
                return (
                  <tr key={item.id} className={`hover:bg-gray-50 transition-colors ${isSelected ? "bg-blue-50/40" : ""}`}>
                    ...
                    {/* Title */}
                    <td className="px-4 py-1.5">
                      <span className="text-[11px] font-bold text-[#364a63]">{item.title}</span>
                    </td>
                    {/* Status */}
                    <td className="px-4 py-1.5">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${STATUS_BADGE[statusKey]}`}>
      ...
      <ConfirmModal
        isOpen={isModalOpen}
        title="Delete Notice?"
        message={selectedIds.length > 0 ? `Delete ${selectedIds.length} item(s)?` : "Delete this notice?"}
