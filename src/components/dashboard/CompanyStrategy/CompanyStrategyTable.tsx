...
import { Pencil, Trash2, ChevronLeft, ChevronRight, Inbox, SearchX, Target, Download, LucideImage } from "lucide-react";
...
const PAGE_SIZE = 20;
export default function CompanyStrategyTable({ onEdit, refreshTrigger, searchQuery = "" }: any) {
  ...
  const fetchData = async () => {
    try { setLoading(true); const res = await StrategyServices.getDetails(); setDataList(Array.isArray(res) ? res : res?.results || []); }
    catch { toast.error("Failed to load strategies"); } finally { setLoading(false); }
  };
  useEffect(() => { fetchData(); }, [refreshTrigger]);
  useEffect(() => {
    const q = searchQuery.toLowerCase();
    setFilteredData(dataList.filter((i) => i.mission_statement?.toLowerCase().includes(q) || i.objective?.toLowerCase().includes(q)));
  ...
  const downloadPDF = () => {
    ...
    autoTable(doc, { head: [["S.N.", "Mission", "Objective"]], body: paginated.map((item, i) => [(currentPage - 1) * PAGE_SIZE + i + 1, item.mission_statement?.slice(0, 60) + "...", item.objective?.slice(0, 60) + "..."]), startY: 25, styles: { fontSize: 8 }, headStyles: { fillColor: [54, 74, 99] } });
    doc.save("CompanyStrategy.pdf"); toast.success("PDF Downloaded");
  };
  ...
  return (
    <div className="space-y-3">
          ...
          <table className="w-full text-left border-separate border-spacing-0">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? <TableLoadingSkeleton rows={5} cols={7} /> : paginated.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-16"><div className="flex flex-col items-center gap-2">{searchQuery ? <SearchX size={32} className="text-rose-300" /> : <Inbox size={32} className="text-gray-200" />}<span className="text-sm font-bold text-[#364a63]">{searchQuery ? "No results." : "No strategies yet."}</span></div></td></tr>
                ...
                return (
                  <tr key={item.id} className={`hover:bg-gray-50 transition-colors ${isSelected ? "bg-blue-50/40" : ""}`}>
                    ...
                    <td className="px-4 py-1.5"><span className="text-[10px] text-[#8094ae] line-clamp-2 max-w-[160px]">{item.mission_statement}</span></td>
                    <td className="px-4 py-1.5"><span className="text-[10px] text-[#8094ae] line-clamp-2 max-w-[160px]">{item.objective}</span></td>
                    <td className="px-4 py-1.5"><span className="text-[10px] text-[#8094ae] line-clamp-2 max-w-[160px]">{item.goals}</span></td>
      ...
      <ConfirmModal isOpen={isModalOpen} title="Delete Strategy?" message={selectedIds.length > 0 ? `Delete ${selectedIds.length} strategies?` : "Delete this strategy?"} onConfirm={handleConfirmDelete} onCancel={() => { setIsModalOpen(false); setDeleteId(null); }} loading={deleteLoading} />
    </div>
  );
}