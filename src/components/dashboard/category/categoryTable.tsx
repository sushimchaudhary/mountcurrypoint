...
export default function CategoryTable({ onEdit, refreshTrigger, searchQuery = "" }: any) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await Promise.all(ids.map((id) => CategoryServices.deleteDetails(id)));
      toast.success(`${ids.length} category(s) deleted`);
  ...
  const downloadPDF = () => {
    ...
    autoTable(doc, {
      head: [["S.N.", "Name", "Menu Items", "Created"]],
  ...
  return (
    <div className="space-y-3">
          ...
          <table className="w-full text-left border-separate border-spacing-0">
            <thead className="sticky top-0 z-30 shadow-sm">
              <tr className="bg-[#f5f6fa]">
                ...
                <th className="px-4 py-1.5 text-[11px] font-bold text-[#8094ae] uppercase">Menu Items</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                const menuItems: any[] = item.menu_items || [];
                ...
                return (
                  <React.Fragment key={item.id}>
                    <tr className={`hover:bg-gray-50 transition-colors ${isSelected ? "bg-blue-50/40" : ""}`}>
                      ...
                      {/* Menu Items Count + expand toggle */}
                      <td className="px-4 py-1.5">
                        {menuItems.length > 0 ? (
                          <button
                            onClick={() => toggleExpand(item.id)}
                            className="flex items-center gap-1 text-[11px] font-bold text-blue-500 hover:text-blue-700 transition-colors"
                          >
                            {isExpanded
                              ? <ChevronDown size={12} />
                    ...
                    {/* Expanded menu items sub-rows */}
                    {isExpanded && menuItems.map((mi: any) => (
                      <tr key={`mi-${mi.id}`} className="bg-gray-50/60">
                        <td colSpan={2} />
                        <td className="px-4 py-1 pl-8" colSpan={1}>
                          <div className="flex items-center gap-2">
                            {/* Thumbnail */}
                            <div className="w-7 h-7 rounded border border-gray-200 bg-white overflow-hidden flex-shrink-0 flex items-center justify-center">
                              {mi.image ? (
                            ...
                            <span className="text-[11px] text-[#364a63] font-medium">{mi.name}</span>
                          </div>
                        </td>
                        <td className="px-4 py-1">
                          <div className="flex flex-wrap gap-1">
                            {mi.portions?.map((p: any, pi: number) => (
                              <span key={pi} className="text-[10px] bg-white border border-gray-200 text-[#526484] px-1.5 py-0.5 rounded font-medium whitespace-nowrap">
                        ...
                        <td className="px-4 py-1">
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${STATUS_BADGE[mi.status] || "bg-gray-100 text-gray-500"}`}>
                            {mi.status === "available" ? "Available" : "Unavailable"}
                          </span>
                        </td>
                        <td />
                      </tr>
      ...
      <ConfirmModal
        ...
        message={selectedIds.length > 0 ? `Delete ${selectedIds.length} categories?` : "Delete this category? All linked menu items will also be removed."}
