...
export default function CompanyOverviewTable({
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await CompanyOverviewServices.getDetails();
  ...
  useEffect(() => {
    ...
    setFilteredData(
      dataList.filter((i) => i.description?.toLowerCase().includes(q)),
    );
    setCurrentPage(1);
    setSelectedIds([]);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await Promise.all(
        ids.map((id) => CompanyOverviewServices.deleteDetails(id)),
      );
      toast.success(`${ids.length} overview(s) deleted`);
  ...
  const downloadPDF = () => {
    const doc = new jsPDF();
    doc.text("Company Overview", 14, 15);
    autoTable(doc, {
      head: [["S.N.", "Description"]],
  ...
  return (
    <div className="space-y-3">
                  ...
                  <td colSpan={5} className="text-center py-16">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-[#364a63]">
                        {searchQuery ? "No results." : "No overviews yet."}
                      </span>
                    </div>
                  </td>
                </tr>
                    ...
                    >
                      ...
                      <td className="px-4 py-1.5">
                         <div className="w-14 h-10 rounded border border-gray-100 overflow-hidden bg-gray-50 flex items-center justify-center">
                          {item.company_img || item.company ? (
                            <AntImage
                              src={item.company_img || item.company}
      ...
      <ConfirmModal
        ...
        message={
          selectedIds.length > 0
            ? `Delete ${selectedIds.length} overviews?`
