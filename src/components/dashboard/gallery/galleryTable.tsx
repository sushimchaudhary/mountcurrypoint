...
export default function GalleryTable({
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await GalleryServices.getDetails();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await Promise.all(ids.map((id) => GalleryServices.deleteDetails(id)));
  ...
  const downloadPDF = () => {
    const doc = new jsPDF();
    doc.text("Gallery List", 14, 15);
    autoTable(doc, {
      head: [["S.N.", "Title", "Has Video", "Created At"]],
      body: paginated.map((item, i) => [
        (currentPage - 1) * PAGE_SIZE + i + 1,
        item.title,
        item.video_url ? "Yes" : "No",
        item.created_at
                  ...
                  <td colSpan={7} className="text-center py-16">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-[#364a63]">
                        {searchQuery ? "No results." : "No gallery items."}
                      </span>
                    </div>
                  </td>
                </tr>
