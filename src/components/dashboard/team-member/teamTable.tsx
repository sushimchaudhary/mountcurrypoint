...
export default function TeamTable({
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await TeamServices.getDetails();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await Promise.all(ids.map((id) => TeamServices.deleteDetails(id)));
                  ...
                  <td colSpan={6} className="text-center py-16">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-[#364a63]">
                        {searchQuery ? "No results." : "No team members."}
                      </span>
                    </div>
                  </td>
                </tr>
                ...
                paginated.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-1.5">
                        <span className="text-[11px] text-[#526484]">
                          {item.position}
                        </span>
                      </td>
                      <td className="px-4 py-1.5">
                        <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-indigo-50 text-indigo-600 text-[11px] font-bold">
                          {item.order}
                        </span>
                      </td>
