...
import { ChevronDown, GlobeLock, Menu, X } from "lucide-react";
...
const NAV_LINKS = [
  { label: "HOME", href: "/" },
  {
    ...
    href: "#",
    children: [
      { label: "Company Introduction", href: "/company-information" },
...
/**
 * Triggers Google Translate programmatically.
 * Google Translate injects a <select> inside #google_translate_element.
 * We find that select and set its value to trigger translation.
 */
...
export default function Navbar() {
  ...
  const [selectedLang, setSelectedLang] = useState(LANGUAGES[1]); // English default
  ...
  return (
    <div className="flex flex-col w-full max-w-7xl mx-auto">
        ...
        <div className="px-4 md:px-10 py-3 flex justify-between items-center gap-4">
            ...
            {NAV_LINKS.map((link) =>
              link.children ? (
                <li key={link.label} className="relative group" ref={aboutRef}>
                  <div className="flex items-center gap-1 px-3 py-2 rounded cursor-pointer hover:text-green-600">
                    {link.label}
                    <svg
                      viewBox="0 0 24 24"
                      fill="none"
                ...
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="block px-3 py-2  rounded hover:text-green-600 transition-colors"
                  >
                    {link.label}
                  </Link>
                </li>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={langRef}>
              ...
              >
                <GlobeLock className="w-4 h-4 text-green-700" /> {/* <--- Lucide Global Icon */}
                <span>{selectedLang.label}</span>
                <ChevronDown className={`w-3 h-3 transition-transform text-green-700 ${langOpen ? "rotate-180" : ""}`} /> {/* <--- Lucide Down Icon */}
              </button>
      ...
      <div className={`lg:hidden bg-white overflow-hidden transition-all duration-500 ease-in-out ${mobileMenuOpen ? "max-h-96" : "max-h-0"}`}>
        <ul className="flex flex-col py-2 border-t">
          {NAV_LINKS.map((link) =>
            link.children ? (
              <li key={link.label}>
                <button onClick={() => setMobileAboutOpen(!mobileAboutOpen)} className="w-full text-left px-6 py-3 font-bold text-green-800">
                  {link.label}
                </button>
                <div className={`bg-green-50 overflow-hidden transition-all duration-300 ${mobileAboutOpen ? "max-h-32" : "max-h-0"}`}>
                  {link.children.map((child) => (
                    <Link key={child.label} href={child.href} className="block px-10 py-2.5 text-sm text-green-700">
                      {child.label}
                    </Link>
                  ))}
                </div>
              ...
              <li key={link.label}>
                <Link href={link.href} className="block px-6 py-3 font-bold text-green-800" onClick={() => setMobileMenuOpen(false)}>
                  {link.label}
                </Link>
              </li>
            )
